...
def compose_preview(foreground_image, background_image, points, closed, offset_x, offset_y):
    ...
    if len(shifted_points) >= 2:
        cv2.polylines(preview, [np.asarray(shifted_points, dtype=np.int32)], isClosed=closed, color=(0, 255, 0), thickness=2)
    return preview


def add_point(foreground_image, points: List[Point], closed: bool, evt: gr.SelectData):
    foreground_image = ensure_rgb(foreground_image)
    if foreground_image is None:
        return None, points, closed, points_to_text(points, closed), gr.update(), gr.update()
    if closed:
        return draw_points(foreground_image, points, closed), points, closed, "Polygon is already closed. Use Clear or Undo to modify it.", gr.update(), gr.update()
    index = getattr(evt, "index", None)
    if index is None or not isinstance(index, (tuple, list)) or len(index) < 2:
        return draw_points(foreground_image, points, closed), points, closed, points_to_text(points, closed), gr.update(), gr.update()
    ...
    return draw_points(foreground_image, points, closed), points, closed, points_to_text(points, closed), gr.update(maximum=max_x), gr.update(maximum=max_y)


def close_polygon(foreground_image, points: List[Point]):
    foreground_image = ensure_rgb(foreground_image)
    closed = len(points) >= 3
...
def undo_point(foreground_image, points: List[Point], closed: bool):
    foreground_image = ensure_rgb(foreground_image)
    if points:
...
def clear_points(foreground_image):
    foreground_image = ensure_rgb(foreground_image)
    points: List[Point] = []
    closed = False
...
def on_foreground_upload(foreground_image):
    foreground_image = ensure_rgb(foreground_image)
    if foreground_image is None:
        return None, [], False, points_to_text([], False), gr.update(maximum=1, value=0), gr.update(maximum=1, value=0)
    return foreground_image, [], False, points_to_text([], False), gr.update(maximum=max(foreground_image.shape[1] - 1, 1), value=0), gr.update(maximum=max(foreground_image.shape[0] - 1, 1), value=0)


def on_background_upload(background_image):
    return ensure_rgb(background_image)
...
def run_blending(foreground_image, background_image, points, closed, offset_x, offset_y, iterations):
    ...
    blended, _, naive = poisson_blend(source_canvas, background_image, shifted_points, iterations=int(iterations), learning_rate=1e-2, device="cpu")
...
def load_examples():
    ...
    preview = fg.copy()
    return fg, bg, preview, bg, [], False, points_to_text([], False), gr.update(maximum=max(fg.shape[1] - 1, 1), value=0), gr.update(maximum=max(fg.shape[0] - 1, 1), value=0)
...
""")
        ...
        foreground_input.upload(on_foreground_upload, inputs=[foreground_input], outputs=[foreground_preview, points_state, closed_state, status_box, offset_x, offset_y], api_name=False, show_api=False)
        background_input.upload(on_background_upload, inputs=[background_input], outputs=[background_preview], api_name=False, show_api=False)
        load_button.click(load_examples, outputs=[foreground_input, background_input, foreground_preview, background_preview, points_state, closed_state, status_box, offset_x, offset_y], api_name=False, show_api=False)
        foreground_preview.select(add_point, inputs=[foreground_input, points_state, closed_state], outputs=[foreground_preview, points_state, closed_state, status_box, offset_x, offset_y], api_name=False, show_api=False)
        close_button.click(close_polygon, inputs=[foreground_input, points_state], outputs=[foreground_preview, closed_state, status_box], api_name=False, show_api=False).then(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        undo_button.click(undo_point, inputs=[foreground_input, points_state, closed_state], outputs=[foreground_preview, points_state, closed_state, status_box], api_name=False, show_api=False).then(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        clear_button.click(clear_points, inputs=[foreground_input], outputs=[foreground_preview, points_state, closed_state, status_box], api_name=False, show_api=False).then(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        offset_x.change(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        offset_y.change(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        background_input.change(update_background_preview, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y], outputs=[background_preview], api_name=False, show_api=False)
        blend_button.click(run_blending, inputs=[foreground_input, background_input, points_state, closed_state, offset_x, offset_y, iterations], outputs=[naive_output, blend_output, status_box], api_name=False, show_api=False)
    return demo
