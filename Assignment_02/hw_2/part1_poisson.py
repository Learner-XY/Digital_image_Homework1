...
DEFAULT_POINTS_A1_TO_A2: List[Point] = [
    (69, 101), (103, 78), (167, 77), (201, 99), (194, 149),
    (179, 214), (132, 262), (93, 226), (72, 162),
]

DEFAULT_POINTS_A2_TO_A1: List[Point] = [
    (57, 95), (92, 74), (160, 77), (194, 104), (190, 154),
    (171, 217), (124, 264), (84, 229), (61, 167),
]
...
def cal_laplacian_loss(foreground_img: torch.Tensor, foreground_mask: torch.Tensor, blended_img: torch.Tensor, background_mask: torch.Tensor) -> torch.Tensor:
    kernel = torch.tensor([[0.0, 1.0, 0.0], [1.0, -4.0, 1.0], [0.0, 1.0, 0.0]], dtype=foreground_img.dtype, device=foreground_img.device).view(1, 1, 3, 3)
...
def poisson_blend(source_image: np.ndarray, target_image: np.ndarray, polygon_points: Sequence[Point], iterations: int = 1200, learning_rate: float = 1e-2, device: str = "cpu") -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    if source_image.shape != target_image.shape:
        target_image = cv2.resize(target_image, (source_image.shape[1], source_image.shape[0]), interpolation=cv2.INTER_AREA)
    ...
    result_crop = blended.detach().clamp(0.0, 1.0).cpu().squeeze(0).permute(1, 2, 0).numpy()
...
def run_case(source_path: Path, target_path: Path, points: Sequence[Point], output_dir: Path, prefix: str, iterations: int, learning_rate: float, device: str) -> None:
    source = load_image(source_path)
    target = load_image(target_path)
    blended, mask, naive = poisson_blend(source, target, points, iterations, learning_rate, device)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--run-both-examples", action="store_true", help="Run a1->a2 and a2->a1 with built-in polygon points.")
    return parser
...
def main() -> None:
    ...
    if args.run_both_examples:
        run_case(Path("base/a1.png"), Path("base/a2.png"), DEFAULT_POINTS_A1_TO_A2, args.output_dir, "a1_to_a2", args.iterations, args.lr, args.device)
        run_case(Path("base/a2.png"), Path("base/a1.png"), DEFAULT_POINTS_A2_TO_A1, args.output_dir, "a2_to_a1", args.iterations, args.lr, args.device)
        return
    points = parse_points(args.points, DEFAULT_POINTS_A1_TO_A2)
    run_case(args.source, args.target, points, args.output_dir, args.prefix, args.iterations, args.lr, args.device)
