...
class CMPFacadeDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        return {"input": input_tensor, "target": target_tensor, "stem": record.stem}
...
class UNetGenerator(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 3, base_channels: int = 32):
        ...
        self.final = nn.Sequential(nn.ConvTranspose2d(base_channels * 2, out_channels, kernel_size=4, stride=2, padding=1), nn.Tanh())
...
def evaluate_model(generator: nn.Module, dataloader: DataLoader, device: torch.device, l1_loss: nn.Module) -> dict:
    generator.eval()
    total_l1 = 0.0
    total_psnr = 0.0
    count = 0
    with torch.no_grad():
        for batch in dataloader:
            ...
            count += 1
    generator.train()
    return {"val_l1": total_l1 / max(count, 1), "val_psnr": total_psnr / max(count, 1)}
...
def train(args: argparse.Namespace) -> None:
    ...
    train_dataset = CMPFacadeDataset(train_records, image_size=args.image_size, load_size=args.load_size, augment=not args.disable_augment)
    ...
    scheduler_g = torch.optim.lr_scheduler.LambdaLR(
        optimizer_g,
        lr_lambda=lambda epoch: 1.0 if epoch < max(args.epochs // 2, 1) else max(0.0, 1.0 - (epoch - max(args.epochs // 2, 1)) / max(args.epochs - max(args.epochs // 2, 1), 1)),
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_summary = {"epoch": epoch, "train_g_loss": epoch_g / max(step_count, 1), "train_d_loss": epoch_d / max(step_count, 1), **metrics}
        ...
        if metrics["val_l1"] < best_val_l1:
            ...
            torch.save({"generator": generator.state_dict(), "discriminator": discriminator.state_dict(), "args": vars(args), "history": history}, checkpoints_dir / "best.pt")
        scheduler_g.step()
        scheduler_d.step()

    torch.save({"generator": generator.state_dict(), "discriminator": discriminator.state_dict(), "args": vars(args), "history": history}, checkpoints_dir / "last.pt")
    with torch.no_grad():
        generator.eval()
