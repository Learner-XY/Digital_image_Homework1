...
def render_marked_image(base_img, src_pts, dst_pts):
    ...
    for i in range(pair_num):
        ...
        if is_anchor:
            cv2.circle(marked, (sx, sy), 5, (255, 255, 0), -1)
            cv2.putText(marked, f"A{i+1}", (sx + 5, sy - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1, cv2.LINE_AA)
        else:
            cv2.circle(marked, (sx, sy), 4, (255, 0, 0), -1)
            cv2.circle(marked, (tx, ty), 4, (0, 0, 255), -1)
            cv2.putText(marked, f"S{i+1}", (sx + 5, sy - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
            cv2.putText(marked, f"T{i+1}", (tx + 5, ty - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
    ...
    if len(src_pts) > len(dst_pts):
        ...
        cv2.circle(marked, (sx, sy), 4, (255, 0, 0), -1)
        cv2.putText(marked, f"S{len(src_pts)}", (sx + 5, sy - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
    return marked
...
def mls_affine_warp(image, source_pts, target_pts, alpha=1.0, eps=1e-8):
    ...
    V = np.stack([grid_x, grid_y], axis=-1)

    diff = V[:, :, None, :] - dst[None, None, :, :]
    d2 = np.sum(diff * diff, axis=-1)
    exact_match = d2 < eps
    weights = 1.0 / np.power(d2 + eps, alpha)
    ...
    phat = dst[None, None, :, :] - p_star[:, :, None, :]
    qhat = src[None, None, :, :] - q_star[:, :, None, :]

    C = np.einsum("hwn,hwna,hwnb->hwab", weights, phat, phat)
    B = np.einsum("hwn,hwna,hwnb->hwab", weights, phat, qhat)
...
def upload_image(img):
    ...
    return img, display_image.copy(), "图片上传成功。在左下图中按 源点 → 目标点 的顺序点击。如果要加固定点，请在同一个位置连续点两次。"
...
def run_warping():
    ...
    warped = mls_affine_warp(image, source_pts=src_orig, target_pts=dst_orig, alpha=1.0)
...
with gr.Blocks() as demo:
    gr.Markdown("## 图像拉伸变形（MLS）")
    gr.Markdown("先上传图片，再在左下图中按 **源点 → 目标点** 的顺序依次点击。如果要添加固定点，请在同一个位置连续点击两次。")
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(label="上传图片", interactive=True, type="numpy", height=220)
            point_select = gr.Image(label="点击选择源点和目标点", interactive=True, type="numpy", height=500)
        with gr.Column():
            result_image = gr.Image(label="变形结果", type="numpy", height=500)
    status_box = gr.Textbox(label="当前状态", value="请先上传图片。", interactive=False)
    with gr.Row():
        run_button = gr.Button("Run Warping")
        clear_button = gr.Button("Clear Points")

    input_image.upload(upload_image, inputs=input_image, outputs=[input_image, point_select, status_box])
    point_select.select(record_points, inputs=None, outputs=[point_select, status_box])
    run_button.click(run_warping, inputs=None, outputs=[result_image, status_box])
    clear_button.click(clear_points, inputs=None, outputs=[point_select, result_image, status_box])
